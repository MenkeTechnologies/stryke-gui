...
| `GUI::format_hotkey(\%parts)` | `{ hotkey, modifiers, key }` — canonicalize parts (W3C names Control/Alt/Shift/Meta, deduped + ordered) into one accelerator; inverse of parse_hotkey; takes `{ keys }` or `{ modifiers, key }` |
